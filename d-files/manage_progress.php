...
if(isset($_GET['id'])){
    // Synchronization: Lock the user_productivity table for reading
    $conn->query("LOCK TABLES user_productivity READ");
...
<script>
    ...
    $('#manage-progress').submit(function(e){
        ...
        // Simulate multithreading: Run background task processing
...
We used shell_exec() to run a background PHP process (process_task.php) for parallel processing.
Synchronization:
We locked the user_productivity table when fetching data using LOCK TABLES and unlocked it after reading to avoid race conditions.